...
# ---------------------------------------------------------------------------
# Endpoint: health check (pentru Render)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Endpoint-uri: utilizator curent
# ---------------------------------------------------------------------------

@app.get("/utilizatori/eu")
def profil_utilizator_curent(
    utilizator_curent=Depends(get_utilizator_curent),
):
    """Returnează informații despre utilizatorul autentificat (fără parola_hash)"""
    return {
...
def creeaza_sarcina(
    ...
    cursor = db.execute(
        "INSERT INTO sarcini (titlu, descriere, data_crearii, utilizator_id) VALUES (?, ?, ?, ?)",
        (sarcina.titlu, sarcina.descriere, datetime.now().isoformat(), utilizator_curent["id"]),
    )
    db.commit()
...
def finalizeaza_sarcina(
    ...
    db.execute(
        "UPDATE sarcini SET finalizata = 1 WHERE id = ?", (sarcina_id,)
    )
    db.commit()
    return dict(db.execute("SELECT * FROM sarcini WHERE id = ?", (sarcina_id,)).fetchone())
...
# ---------------------------------------------------------------------------
# Servirea frontend-ului (StaticFiles) - TREBUIE SĂ FIE ULTIMUL
# ---------------------------------------------------------------------------
